...
enum OutlinerTree {
    ...
    /// Fold/unfold self-check used by the shell proof path — returns `nil` on success.
    static func proofFailure() -> String? {
        let pupil = OutlinerItem(
            id: NodeID(kind: .mesh, index: 4),
            name: "Pupil",
            depth: 2
        )
        let eyes = OutlinerItem(
            id: NodeID(kind: .mesh, index: 1),
            name: "Eyes",
            depth: 1,
            children: [pupil]
        )
        let beak = OutlinerItem(
            id: NodeID(kind: .mesh, index: 2),
            name: "Beak",
            depth: 1
        )
        let body = OutlinerItem(
            id: NodeID(kind: .mesh, index: 0),
            name: "Body",
            depth: 0,
            children: [eyes, beak]
        )
        ...
        var expanded = defaultExpandedIDs(roots: roots, maxDepth: 3)
        let openNames = visibleItems(roots: roots, expanded: expanded).map(\.name)
        guard openNames == ["Body", "Eyes", "Pupil", "Beak"] else {
            return "default expand visible \(openNames)"
        }

        toggle(body, recursive: false, expanded: &expanded)
        let collapsed = visibleItems(roots: roots, expanded: expanded).map(\.name)
        guard collapsed == ["Body"] else {
            return "collapse Body visible \(collapsed)"
        }

        toggle(body, recursive: false, expanded: &expanded)
        toggle(eyes, recursive: false, expanded: &expanded)
        let eyesClosed = visibleItems(roots: roots, expanded: expanded).map(\.name)
        guard eyesClosed == ["Body", "Eyes", "Beak"] else {
            return "collapse Eyes visible \(eyesClosed)"
        }

        toggle(body, recursive: true, expanded: &expanded)
        let finished = visibleItems(roots: roots, expanded: expanded).map(\.name)
        guard finished == ["Body", "Eyes", "Pupil", "Beak"] else {
            return "option finish-expand visible \(finished)"
        }

        let skipped = meshRoots([
            SceneNode(
                kind: .empty,
                index: 9,
                name: "Armature",
                children: [
                    SceneNode(
                        ...
                        index: 0,
                        name: "Body",
                        children: [SceneNode(kind: .mesh, index: 1, name: "Eyes")]
                    )
                ]
            )
        ])
        guard skipped.count == 1,
              skipped[0].name == "Body",
              skipped[0].depth == 0,
              skipped[0].children.count == 1,
              skipped[0].children[0].name == "Eyes",
              skipped[0].children[0].depth == 1
        else {
            return "skip-non-mesh tree \(skipped)"
        }
